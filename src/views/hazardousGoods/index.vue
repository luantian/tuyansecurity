...
<template>
  <div class="hazardous_main">
    <div class="hazardous_main_left">
      ...
      <div class="main_left_top">
        <el-button
          type="primary"
          size="mini"
          @click="addHazardousGoodType(null)"
          >新增</el-button
        >
      </div>
      <el-tree
    ...
    <div class="hazardous_main_right">
      ...
      <div class="add_btn">
        <el-button type="primary" size="mini" @click="addHazardousGood(null)"
          >新增</el-button
        >
      </div>
      <div class="main_right_bottom">
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <el-button type="text" @click="updatedHazardousGood(scope.row)"
                >修改</el-button
              >
              <el-button type="text" @click="deleteHazardousGood(scope.row)"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      >
        <el-form
          ...
          :rules="typeFormRules"
        >
          <el-form-item label="分类名称" prop="dr_name">
            <el-input
              v-model="addTypeForm.dr_name"
              size="small"
              clearable
            ></el-input>
          </el-form-item>
        ...
        <div class="center mt20">
          <el-button
            ...
            >保存</el-button
          >
          <el-button @click="showAddType = false" size="small">取消</el-button>
        </div>
      </el-dialog>
      <!-- 新增分类 end -->
      <!-- 修改分类 start -->
      ...
      >
        ...
        <div class="center mt20">
          <el-button
            ...
            >保存</el-button
          >
          <el-button @click="showEditType = false" size="small">取消</el-button>
        </div>
      </el-dialog>
      <!-- 修改分类 end -->
      ...
      >
        ...
        >
          ...
          >
            <el-select size="small" v-model="addForm.dr_big_category">
              <el-option
                :value="item.dr_id"
                :label="item.dr_name"
          ...
          <el-form-item label="描述" label-width="130px">
            <el-input
              v-model="addForm.dr_desc"
              size="small"
              clearable
            ></el-input>
          </el-form-item>
          <el-form-item label="责任人" label-width="130px">
            <el-input
              v-model="addForm.dr_duty"
              size="small"
              clearable
            ></el-input>
          </el-form-item>
      ...
      >
        ...
        >
          ...
          >
            <el-input
              v-model="editForm.dr_point"
              size="small"
              clearable
            ></el-input>
          </el-form-item>
          <el-form-item label="易燃易爆名称" label-width="130px" prop="dr_name">
            <el-input
              v-model="editForm.dr_name"
              size="small"
              clearable
            ></el-input>
          </el-form-item>
          <el-form-item
            ...
            prop="dr_big_category"
          >
            <el-select size="small" v-model="editForm.dr_big_category">
              <el-option
                :value="item.dr_id"
                :label="item.dr_name"
                v-for="item in mapList1"
  ...
  methods: {
    ...
    getMapCode() {
      return new Promise((reslove, reject) => {
        this.$get("/v1/dr/mapcode-list/92")
          .then((res) => {
    ...
    commitAddHazardousGoodType() {
      this.$refs.addTypeForm.validate((valid) => {
        if (valid) {
          this.$post("v1/dr/dangerous-tree-add", this.addTypeForm).then(
            (res) => {
              console.log(res);
    ...
    commitUpdateHazardousGoodTye(isEdit) {
      this.$refs.editTypeForm.validate((valid) => {
        if (valid) {
          this.$post("v1/dr/dangerous-tree-update", this.editTypeForm).then(
            (res) => {
              console.log(res);
    ...
    commitAddHazardousGood() {
      ...
      this.$refs.addForm.validate((valid) => {
        ...
        // addForm.dr_unit.length &&
        //   (addForm.dr_unit = addForm.dr_unit[addForm.dr_unit.length - 1]);
        if (typeof this.addForm.dr_unit === "object") {
          this.addForm.dr_unit =
            this.addForm.dr_unit[this.addForm.dr_unit.length - 1];
        }
        if (valid) {
    ...
    commitUpdateHazardousGood(isEdit) {
      this.$refs.editForm.validate((valid) => {
        if (valid) {
          if (typeof this.editForm.dr_unit === "object") {
            this.editForm.dr_unit =
              this.editForm.dr_unit[this.editForm.dr_unit.length - 1];
          }
          this.$post("/v1/dr/dangerous-update", this.editForm).then((res) => {
            // let editForm = JSON.parse(JSON.stringify(this.editForm));
            // editForm.dr_unit.length &&
            //   (editForm.dr_unit = editForm.dr_unit[editForm.dr_unit.length - 1]);
