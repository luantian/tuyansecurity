...
<div class="dashboard-container">
  <el-row :gutter="20" type="flex">
    ...
    <el-col :span="24" style="flex:1">
            ...
            <el-row :gutter="20">
              <el-col :span="8">
                <el-form-item label="设备名称">
                  <el-input clearable size="small" v-model="listQuery.dr_facilities_name" placeholder="请输入" />
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="组件">
                  <el-input clearable size="small" v-model="listQuery.dr_component" placeholder="请输入" />
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label=" ">
                  <el-button type="primary" size="small" @click="handleSearch">过滤</el-button>
      ...
      <!-- 表格 -->
      <el-table :data="tableData" :height="tableHeight" border size="small" v-loading="loading" :header-cell-style="{background:'#f5f7fa',color:'#606266'}" highlight-current-row style="width: 100%" empty-text="暂无数据" @selection-change="handleSelectionChange">
        <!-- <el-table-column type="selection" align="center" fixed width="55" /> -->
        <el-table-column prop="dr_facilities_name" label="消防设施名称" align="center" show-overflow-tooltip>
        </el-table-column>
        <el-table-column prop="dr_component" label="组件" align="center" show-overflow-tooltip />
        ...
        <el-table-column label="操作" width="200" align="center">
          <template slot-scope="scope">
            ...
            <el-button type="primary" size="small" @click="handleDelete(scope.row)" v-if="scope.row.dr_status==0&&!scope.row.dr_device_serial">启用</el-button>
            <el-button type="danger" size="small" @click="handleDelete(scope.row)" v-if="scope.row.dr_status==1&&!scope.row.dr_device_serial">删除</el-button>
  ...
  <!-- 新增 -->
  <el-dialog :close-on-click-modal="false" :title="isEdit?'编辑消防设施':'添加消防设施'" :visible.sync="dialogVisible" width="520px">
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="120px" class="demo-ruleForm">
      <el-form-item label="消防设施名称" prop="drFacilitiesName">
        <el-input clearable size="small" v-model="ruleForm.drFacilitiesName"></el-input>
      </el-form-item>
      <el-form-item label="组件" prop="drComponent">
        <el-input clearable size="small" v-model="ruleForm.drComponent"></el-input>
      </el-form-item>
      <el-form-item label="所属单位" prop="drUnitId">
        <el-cascader :options="units" :props="{
              checkStrictly: true,
              value: 'dr_unit_id',
              label: 'dr_unit_name',
              children: 'dr_son',
            }" clearable v-model="ruleForm.drUnitId" size="small"></el-cascader>
      </el-form-item>
      <el-form-item label="上传图片" prop="drPic">
        <el-upload class="upload-demo avatar-uploader" ref="upload" :action="`/v1/dr/upload-point`" accept="image/*" :on-preview="handlePreview" :before-remove="beforeRemove" :headers="{ token: $bus.user.token }" :show-file-list="false" :on-success="
                        (res) => {
        ...
        <img v-if="ruleForm.drPic" :src="ruleForm.drPic" style="margin-top: 20px; height: 180px" />
      </el-form-item>
    </el-form>
